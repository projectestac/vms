...
sudo apt-get install -y --force-yes php7.0-dev &> /dev/null
...
echo "extension=oci8.so" | sudo tee -a /etc/php/7.0/mods-available/oci8.ini

sudo ln -s /etc/php/7.0/mods-available/oci8.ini /etc/php/7.0/apache2/conf.d/30-oci8.ini
sudo mv /etc/php/7.0/cli/conf.d/ /etc/php/7.0/cli/conf.d_old
sudo ln -s /etc/php/7.0/apache2/conf.d /etc/php/7.0/cli/conf.d
